...
export default async function LoginPage() {
  // Fetch the CSRF token server-side so the form works without any client JS
  const host = headers().get('host') ?? 'localhost:3000';
  const proto = process.env.NODE_ENV === 'production' ? 'https' : 'http';
  const csrfRes = await fetch(`${proto}://${host}/api/auth/csrf`);
    ...
    <main className="min-h-screen flex items-center justify-center px-4">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 w-full max-w-md text-center">
        ...
        <form action="/api/auth/signin/github" method="POST" className="mt-8">
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 py-3 px-6 rounded-lg font-semibold hover:bg-gray-100 transition"
          >
